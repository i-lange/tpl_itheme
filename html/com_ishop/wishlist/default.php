...
<?php endif; ?>
<?php if ($this->wishlist->count > 0) : ?>
    <div class="products__grid mb-5">
        <?php foreach ($this->wishlist->products as $product) : ?>
            <?php echo LayoutHelper::render('itheme.product.small', ['item' => $product, 'params' => $this->params]) ?>
        <?php endforeach; ?>
    </div>
<?php else : ?>
    <div class="module-cart-empty mb-5">
        <?php echo LayoutHelper::render('itheme.icon', ['icon' => 'i-like', 'class' => 'mega primary']); ?>
        <p><?php echo Text::_('COM_ISHOP_WISHLIST_NULL'); ?></p>
    </div>
<?php endif; ?>
