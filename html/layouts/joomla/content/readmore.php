...
<?php if (!$params->get('access-view')) : ?>
    <a class="btn btn-primary stretched-link" href="<?php echo $displayData['link']; ?>" aria-label="<?php echo Text::_('JGLOBAL_REGISTER_TO_READ_MORE') . ' ' . $this->escape($item->title); ?>">
        <?php echo Text::_('JGLOBAL_REGISTER_TO_READ_MORE'); ?><?php echo LayoutHelper::render('itheme.icon', ['icon' => 'i-chevron-'. $direction]); ?>
    </a>
...
<?php elseif ($params->get('show_readmore_title', 0) == 0) : ?>
    <a class="btn btn-link link-primary ps-0 text-decoration-none stretched-link" href="<?php echo $displayData['link']; ?>" aria-label="<?php echo Text::sprintf('JGLOBAL_READ_MORE_TITLE', $this->escape($item->title)); ?>">
